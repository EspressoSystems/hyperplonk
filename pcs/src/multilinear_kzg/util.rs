...
//         // we reverse the order here because the coefficient vec are stored
...
// /// polynomials that goes through the points; extend the dimension of the
// points /// by `log(points.len())`.
...
//     // 1.1 build the indexes and the univariate polys that go through the
// indexes     let indexes: Vec<Vec<bool>> = (0..points.len())
...
//         uni_polys.push(Evaluations::from_vec_and_domain(eval,
// *domain).interpolate());     }
...
mod test {
    ...
    //         // 0, 1 |-> 2
    //         // 1, 1 |-> 5
    //         let w_eval = vec![F::zero(), F::zero(), F::from(2u64),
    // F::from(5u64)];         let w =
    // DenseMultilinearExtension::from_evaluations_vec(2, w_eval);

    //         // l0 =   t + 2
    //         // l1 = -2t + 4
    //         let l0 = DensePolynomial::from_coefficients_vec(vec![F::from(2u64),
    // F::one()]);         let l1 =
    // DensePolynomial::from_coefficients_vec(vec![F::from(4u64), -F::from(2u64)]);
    ...
    //         let l0 = DensePolynomial::from_coefficients_vec(vec![F::from(2u64),
    // F::one()]);         let l1 =
    // DensePolynomial::from_coefficients_vec(vec![-F::from(4u64), F::from(3u64)]);
    //         let l2 = DensePolynomial::from_coefficients_vec(vec![F::from(6u64),
    // -F::from(5u64)]);         let res = compute_w_circ_l_with_prefix(&w, [l0,
    // l1, l2].as_ref(), 8)?;
    ...
    //         let l = build_l_with_prefix(&[point1.clone(), point2.clone()],
    // &domain)?;
    ...
    //             -Fr::one() / Fr::from(2u64),
    //         ]);
    //         let l1 = DensePolynomial::from_coefficients_vec(vec![Fr::from(2u64),
    // -Fr::one()]);         let l2 =
    // DensePolynomial::from_coefficients_vec(vec![Fr::from(3u64), -Fr::one()]);
...
//     // Example from page 53:
//     // W1 = 3x1x2 + 2x2
//     let w_eval = vec![Fr::zero(), Fr::from(2u64), Fr::zero(),
// Fr::from(5u64)];     let w1 =
// Rc::new(DenseMultilinearExtension::from_evaluations_vec(2, w_eval));

//     // W2 = x1x2 + x1
//     let w_eval = vec![Fr::zero(), Fr::zero(), Fr::from(1u64),
// Fr::from(2u64)];     let w2 =
// Rc::new(DenseMultilinearExtension::from_evaluations_vec(2, w_eval));

//     // W3 = x1 + x2
//     let w_eval = vec![Fr::zero(), Fr::one(), Fr::from(1u64), Fr::from(2u64)];
//     let w3 = Rc::new(DenseMultilinearExtension::from_evaluations_vec(2,
// w_eval));
...
//         let point: Vec<Fr> = l.iter().map(|poly|
// poly.evaluate(&r)).collect();
