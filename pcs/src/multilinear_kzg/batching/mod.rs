...
//     util::{build_l,
//         // compute_w_circ_l_with_prefix,
//         merge_polynomials},
//     verify_internal, MultilinearKzgBatchProof,
// };
// use crate::{
//     multilinear_kzg::util::{
//         // build_l_with_prefix,
//         compute_w_circ_l, get_uni_domain},
...
// use ark_poly::{DenseMultilinearExtension, EvaluationDomain,
// MultilinearExtension, Polynomial}; use ark_std::{end_timer, format, rc::Rc,
// start_timer, string::ToString, vec, vec::Vec}; use transcript::IOPTranscript;
...
// // /// handled at the caller layer, and utilize
// 'multi_open_same_poly_internal' // /// API.
// // ///
...
// // ///
// // /// Steps:
// // /// 1. build `l(points)` which is a list of univariate polynomials that
...
// //     if points_len == 0 {
// //         return Err(PCSError::InvalidParameters("points is
// empty".to_string())); //     }
...
// //     let q_x_commit = UnivariateKzgPCS::<E>::commit(uni_prover_param,
// &q_x)?; //     transcript.append_serializable_element(b"q(x)", &q_x_commit)?;
...
// //             UnivariateKzgPCS::<E>::open(uni_prover_param, &q_x,
// &domain.element(i))?; //         q_x_opens.push(q_x_open);
...
// //     // 6. build q(r) and its opening
// //     let (q_x_open, q_r_value) =
// UnivariateKzgPCS::<E>::open(uni_prover_param, &q_x, &r)?; //     q_x_opens.
// push(q_x_open); //     q_x_evals.push(q_r_value);
...
// //         .collect();
// //     // 8. output an opening of `w` over point `p`
// //     let (mle_opening, mle_eval) = open_internal(ml_prover_param,
// &merge_poly, &point)?;

// //     // 9. output value that is `w` evaluated at `p` (which should match
// `q(r)`) //     if mle_eval != q_r_value {
// //         return Err(PCSError::InvalidProver(
...
// //     transcript.append_serializable_element(b"q(x)",
// &batch_proof.q_x_commit)?; //     // 2. sample `r` from transcript
// //     let r = transcript.get_and_append_challenge(b"r")?;
...
// //         return Ok(false);
// //     }
// //     // 4. build `l(points)` which is a list of univariate polynomials that
// goes //     // through the points
// //     let uni_polys = build_l_with_prefix(points, &domain)?;

// //     // 5. get a point `p := l(r)`
// //     let point: Vec<E::Fr> = uni_polys.iter().map(|x|
// x.evaluate(&r)).collect(); //     // 6. verifies `p` is valid against
// multilinear KZG proof //     let res = verify_internal(
// //         ml_verifier_param,
// //         multi_commitment,
// //         &point,
// //         &values[points_len],
...
// /// 8. output an opening of `w` over point `p`
// /// 9. output `w(p)`
// pub(super) fn multi_open_same_poly_internal<E: PairingEngine>(
//     uni_prover_param: &UnivariateProverParam<E::G1Affine>,
//     ml_prover_param: &MultilinearProverParam<E>,
...
//     // 6. build q(r) and its opening
//     let (q_x_open, q_r_value) = UnivariateKzgPCS::<E>::open(uni_prover_param,
// &q_x, &r)?;     q_x_opens.push(q_x_open);
...
//         .collect();
//     // 8. output an opening of `w` over point `p`
//     let (mle_opening, mle_eval) = open_internal(ml_prover_param, &polynomial,
// &point)?;
...
//             generate_evaluations_single_poly,
//         },
...
//     //     let com = MultilinearKzgPCS::multi_commit(&(ml_ck.clone(),
// uni_ck.clone()), polys)?;     //     let (batch_proof, evaluations) =
...
//     //             q_x_commit: Commitment(<E as
// PairingEngine>::G1Affine::default()),     //             q_x_opens: vec![],
//     //         },
//     //     )
//     //     .is_err());
...
// PairingEngine>::G1Affine::default());     //
// assert!(!batch_verify_internal(     //         &uni_vk,
//     //         &ml_vk,
//     //         &com,
//     //         &points,
//     //         &evaluations,
...
//     //     let uni_params =
//     //         UnivariateUniversalParams::<E>::gen_srs_for_testing(&mut rng,
// 1usize << 15)?;     //     let ml_params =
// MultilinearUniversalParams::<E>::gen_srs_for_testing(&mut rng, 15)?;     //
// for num_poly in 2..10 {     //         for nv in 1..5 {
//     //             let polys1: Vec<_> = (0..num_poly)
//     //                 .map(|_| Rc::new(DenseMultilinearExtension::rand(nv,
// &mut rng)))     //                 .collect();
//     //             test_multi_open_helper(&uni_params, &ml_params, &polys1,
// &mut rng)?;     //         }
//     //     }
...
//         let com = MultilinearKzgPCS::commit(&(ml_ck.clone(), uni_ck.clone()),
// poly)?;         let (batch_proof, evaluations) =
//             multi_open_same_poly_internal(&uni_ck, &ml_ck, poly, &com,
// &points)?;
...
//                 // normal polynomials
//                 let polys1 = Rc::new(DenseMultilinearExtension::rand(nv, &mut
// rng));                 test_same_poly_multi_open_internal_helper(
//                     &uni_params,
//                     &ml_params,
//                     &polys1,
//                     point_len,
